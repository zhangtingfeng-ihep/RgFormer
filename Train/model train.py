...
# Set up logging
logging.basicConfig(filename='C:\\SASBDB\\model\\33\\train.log', level=logging.INFO,
...
def load_data():
    ...
    for code in list(rg_dict.keys()):
        ...
        try:
            ...
            if len(filtered_data) == 0:
                skipped_codes.append((code, "No valid data after filtering"))
                logging.info(f"Code {code}: Skipped due to no valid data after filtering")
                del rg_dict[code]
                continue

           
            filtered_data[:, 1] = normalize_intensity(filtered_data[:, 1])

            curve_data[code] = filtered_data
            logging.info(f"Code {code}: Loaded {len(filtered_data)} valid rows, I normalized by max")
    ...
    logging.info(f"Train set (original): {len(train_codes)} samples | Val set: {len(val_codes)} samples")
    ...
    for code in train_codes:
        rg = train_rg_dict[code]
        original_data = train_curve_data[code]
        ...
        for i in range(8):  
            ...
            noise = np.random.normal(0, 0.005, aug_data[:, 1].shape).astype(np.float32)
            aug_data[:, 1] = aug_data[:, 1] + noise
            aug_data[:, 1] = np.clip(aug_data[:, 1], 0, None)

            
            aug_data[:, 1] = normalize_intensity(aug_data[:, 1])

            aug_curve_data[aug_code] = aug_data
            aug_rg_dict[aug_code] = rg

    
    train_curve_data.update(aug_curve_data)
    train_rg_dict.update(aug_rg_dict)

    logging.info(f"After augmentation - Train samples: {len(train_curve_data)} "
                 f"(original {len(train_codes)}) | Val samples: {len(val_curve_data)}")

    # ====================== 5. calculate max_length ======================
    lengths = [len(data) for data in curve_data.values()]
    max_length = int(np.percentile(lengths, 95))
    logging.info(f"Dataset prepared. max_length: {max_length}")
...
def train_model():
    ...
    model = ImprovedSAXSModel(input_dim=2, d_model=512, nhead=16, num_layers=4,
                              dim_feedforward=2048, max_length=max_length, dropout=0.2).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_loader:
            ...
            with autocast(device_type='cuda' if device.type == 'cuda' else 'cpu'):  
                output = model(data, mask).squeeze(-1)
            ...
            train_loss += weighted_loss.item() * len(rg)
            train_pred_rgs.extend(output.detach().cpu().numpy())
            train_true_rgs.extend(rg.detach().cpu().numpy())
        ...
        train_mre = np.mean(
            np.abs((np.array(train_true_rgs) - np.array(train_pred_rgs)) / np.array(train_true_rgs))) * 100

        logging.info(f"Epoch {epoch + 1}/{num_epochs} | Train Loss: {train_loss:.6f} | "
                     f"RMSE: {train_rmse:.4f} | MAE: {train_mae:.4f} | R²: {train_r2:.4f} | MRE: {train_mre:.2f}%")

        # Validation
        model.eval()
        val_loss = 0.0
        val_true_rgs, val_pred_rgs = [], []
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                val_loss += weighted_loss.item() * len(rg)
                val_pred_rgs.extend(output.detach().cpu().numpy())
                val_true_rgs.extend(rg.detach().cpu().numpy())
        ...
        val_mre = np.mean(np.abs((np.array(val_true_rgs) - np.array(val_pred_rgs)) / np.array(val_true_rgs))) * 100
        current_lr = optimizer.param_groups[0]['lr']
        scheduler.step(val_loss)
        logging.info(f"Epoch {epoch + 1}/{num_epochs} | Val Loss: {val_loss:.6f} | "
                     f"RMSE: {val_rmse:.4f} | MAE: {val_mae:.4f} | R²: {val_r2:.4f} | MRE: {val_mre:.2f}% | LR: {current_lr:.8f}")
