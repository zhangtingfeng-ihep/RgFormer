...
# Set up logging
logging.basicConfig(filename='C:\\SASBDB\\model\\44\\train.log', level=logging.INFO,
                    format='%(asctime)s - %(levelname)s - %(message)s')

# Define the model class without channel attention, positional encoding, and conv2
class ImprovedSAXSModel(nn.Module):
    def __init__(self, input_dim=2, d_model=512, nhead=16, num_layers=4, dim_feedforward=2048, max_length=2617,
        ...
        self.embedding = nn.Linear(64 * 2, d_model)  # Adjusted for conv1 and conv3
    ...
    def forward(self, x, mask):
        ...
        x = torch.cat([conv1_out, conv3_out], dim=1)  # Concatenate conv1 and conv3
        x = self.pool(x)
        x = x.transpose(1, 2)
