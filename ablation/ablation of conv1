...
# Set up logging
logging.basicConfig(filename='D:\\SASBDB\\model\\43\\train.log', level=logging.INFO,
                    format='%(asctime)s - %(levelname)s - %(message)s')

# Define the model class without channel attention, positional encoding, and conv1
class ImprovedSAXSModel(nn.Module):
    def __init__(self, input_dim=2, d_model=512, nhead=16, num_layers=4, dim_feedforward=2048, max_length=2617,
        ...
        self.embedding = nn.Linear(64 * 2, d_model)  # Adjusted for 2 conv outputs
    ...
    def forward(self, x, mask):
        ...
        x = torch.cat([conv2_out, conv3_out], dim=1)  # Concatenate only conv2 and conv3
        x = self.pool(x)
        x = x.transpose(1, 2)
...
def load_data():
    xml_path = 'D:\\SASBDB\\sasbdb_data.xml'
