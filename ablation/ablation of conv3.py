...
# Set up logging
logging.basicConfig(filename='C:\\SASBDB\\model\\45\\train.log', level=logging.INFO,
                    format='%(asctime)s - %(levelname)s - %(message)s')

# Define the model class without channel attention, positional encoding, and conv3
class ImprovedSAXSModel(nn.Module):
    def __init__(self, input_dim=2, d_model=512, nhead=16, num_layers=4, dim_feedforward=2048, max_length=2617,
        ...
        self.embedding = nn.Linear(64 * 2, d_model)  # Adjusted from 64 * 3 to 64 * 2
    ...
    def forward(self, x, mask):
        ...
        x = torch.cat([conv1_out, conv2_out], dim=1)  # Only include conv1 and conv2
        x = self.pool(x)
        x = x.transpose(1, 2)
...
def train_model():
    ...
    output_dir = 'C:\\SASBDB\\model\\45'  # Changed from 33 to 45
