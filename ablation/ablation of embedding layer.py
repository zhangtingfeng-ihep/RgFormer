...
# Set up logging
logging.basicConfig(filename='C:\\SASBDB\\model\\41\\train.log', level=logging.INFO,
                    format='%(asctime)s - %(levelname)s - %(message)s')

# Define the model class without channel attention, positional encoding, and Embedding layer
class ImprovedSAXSModel(nn.Module):
    def __init__(self, input_dim=2, d_model=512, nhead=16, num_layers=4, dim_feedforward=2048, max_length=2617, dropout=0.2):
        super(ImprovedSAXSModel, self).__init__()
        self.conv1 = nn.Conv1d(input_dim, 171, kernel_size=3, padding=1)
        self.conv2 = nn.Conv1d(input_dim, 171, kernel_size=5, padding=2)
        self.conv3 = nn.Conv1d(input_dim, 170, kernel_size=7, padding=3)  # 171 + 171 + 170 = 512
    ...
    def forward(self, x, mask):
        ...
        x = torch.cat([conv1_out, conv2_out, conv3_out], dim=1)  # (batch, 512, seq_len)
        x = self.pool(x)
        x = x.transpose(1, 2)  # (batch, seq_len//2, 512)
        seq_len_after_pool = x.size(1)

        Mx = mask[:, ::2]
        if Mx.size(1) > seq_len_after_pool:
            Mx = Mx[:, :seq_len_after_pool]
        elif Mx.size(1) < seq_len_after_pool:
            padding = torch.zeros(Mx.size(0), seq_len_after_pool - Mx.size(1)).to(Mx.device)
            Mx = torch.cat([Mx, padding], dim=1)

        x = self.transformer_encoder(x, src_key_padding_mask=Mx == 0)
