...
# Set up logging
logging.basicConfig(filename='C:\\SASBDB\\model\\40\\train.log', level=logging.INFO,
                    format='%(asctime)s - %(levelname)s - %(message)s')

# Define the model class without channel attention, positional encoding, and max pooling
class ImprovedSAXSModel(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        if mask.size(1) > seq_len:
            ...
            padding = torch.zeros(mask.size(0), seq_len - mask.size(1)).to(mask.device)
