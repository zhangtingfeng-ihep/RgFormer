...
# Set up logging
logging.basicConfig(filename='D:\\SASBDB\\model\\39\\train.log', level=logging.INFO,
                    format='%(asctime)s - %(levelname)s - %(message)s')

# Define the model class without channel attention, positional encoding, and global average pooling
class ImprovedSAXSModel(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        # Remove global average pooling, use the last token's output
        x = self.ln(x[:, -1, :])  # Select the last token after transformer
        residual = x
